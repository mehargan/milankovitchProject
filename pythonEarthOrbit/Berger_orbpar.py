...
# C****   OBLIQD = OBLIQ# + sum[A cos(ft+delta)]   Equation 1 (5)
    ...
    SUMC = 0
    for i in np.arange(1,47+1).reshape(-1):
        ARG = piz180 * (ym1950 * t1(i,2) / 3600 + t1(i,3))
        SUMC = SUMC + t1(i,1) * np.cos(ARG)
...
# C****
    ...
    for i in np.arange(1,19+1).reshape(-1):
        ...
        ESINPI = ESINPI + t4(i,1) * np.sin(ARG)
        ECOSPI = ECOSPI + t4(i,1) * np.cos(ARG)
    
    ECCEN = np.sqrt(ESINPI * ESINPI + ECOSPI * ECOSPI)
    ecc = ECCEN
...
# C****
    ...
    FSINFD = 0
    for i in np.arange(1,78+1).reshape(-1):
        ARG = piz180 * (ym1950 * t5(i,2) / 3600 + t5(i,3))
        FSINFD = FSINFD + t5(i,1) * np.sin(ARG)
    
    PSI = piz180 * (3.392506 + (ym1950 * 50.439273 + FSINFD) / 3600)
    OMEGVP = np.mod(PIE + PSI + 0.5 * twopi,twopi)
    omega_bar = OMEGVP
