...
def generate_rot_m(obliquity = None,precession = None): 
...
#orbit model, and no input checking/warnings are performed.
    ...
    v = np.sin(precession)
    w = 0
    tilt_m = np.transpose(np.array([[cos_eps + u ** 2 * onem_coseps,u * v * onem_coseps - w * sin_eps,u * w * onem_coseps + v * sin_eps],[u * v * onem_coseps + w * sin_eps,cos_eps + v ** 2 * onem_coseps,v * w * onem_coseps - u * sin_eps],[u * w * onem_coseps - v * sin_eps,v * w * onem_coseps + u * sin_eps,cos_eps + w ** 2 * onem_coseps]]))
    return tilt_m