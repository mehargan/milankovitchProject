...
def getLaskar(input_yr = None,Lask_neg = None,Lask_pos = None): 
...
#Date: June 21, 2012
    ...
    if not len(q)==0 :
        ...
        if np.abs(precess(qe) - precess(qs)) >= 180:
            #annual step of ~18 degrees/1,000 years
            precess_sol = interp1(np.array([[table_year(qs)],[table_year(qe)]]),np.array([[precess(qs) - 360],[precess(qe)]]),input_yr,'linear',NaN)
            ...
            precess_sol = interp1(np.array([[table_year(qs)],[table_year(qe)]]),np.array([[precess(qs)],[precess(qe)]]),input_yr,'linear',NaN)
    
    return ecc_sol,obliq_sol,precess_sol